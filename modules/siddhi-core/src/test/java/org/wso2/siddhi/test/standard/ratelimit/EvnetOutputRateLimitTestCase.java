...
public class EvnetOutputRateLimitTestCase {
    ...
    public void testEventOutputRateLimitQuery1() throws InterruptedException {
        ...
        Assert.assertEquals("Event arrived", true, eventArrived);//判断两个Object类型的值，他们两个比较的是引用地址是否相等, 并没有对内容进行比较，相等程序继续运行，否则抛出异常。
        ...
        siddhiManager.addCallback(queryReference, new QueryCallback() {
            ...
            public void receive(long timeStamp, Event[] inEvents, Event[] removeEvents) {
                ...
                if (inEvents != null) {
                    ...
                    //传入的inEvents[0].getData0()是否等于ip"192.10.1.5"，是就运行成功。
                } else {
                    Assert.fail("Remove events emitted");
                }
                eventArrived = true;
